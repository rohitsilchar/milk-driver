...
void initState() {
  ...
  getKeysLists(appState.currentLanguageCode.value).then((value) {
    ...
    if ( authController.token.value.toString() != "null" && authController.token.value.isNotEmpty) {
       ...
       Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) =>!gets.hasData("language") ?
       const SelectLanguage(): const LoginScreen()),(route) => false);
    }
    });
   // });
    super.initState();
